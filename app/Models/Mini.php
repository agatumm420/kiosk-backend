...
class Mini extends Model
{
   use CrudTrait;
    ...
    public function displays(){
      return  $this->belongsToMany(Display::class, 'minis_displays','mini_id' ,'display_id');
    }
    public function reorderGroup($crud = false)
    {
        ...
        $buttons = '';
            ...
            foreach ($displays as $display) {
                $buttons .= '<a class="dropdown-item" href="'.url($crud->route.'/reorder').'?'.$group.'">'.$display->name.'</a>';
            }
            $buttons .= '</div>
                </div>
            </div>';
